  ...
  async function getList(e) {
    ...
    try {
      const response = await fetch(
        `https://thingproxy.freeboard.io/fetch/https://scoresaber.com/api/player/${userId}/scores?limit=1&sort=top`,
        {
          headers: {
      ...
      for (let i = 1; i <= lastPage; i++) {
        const response = await fetch(
          `https://thingproxy.freeboard.io/fetch/https://scoresaber.com/api/player/${userId}/scores?limit=50&sort=top&page=${i}`,
          {
            headers: {
              Accept: "application/json",
            },
